...
\usepackage[paperwidth=39cm,paperheight=20cm,
...
%%%
    ...
    \node (userd)  [docbox, right=45mm of compd] { User Documumentation };

    \node (ncsaenct) [docboxm, above=12mm of auxicd ] { NCSA Enclave Test Spec };
    \node (baseenct) [docboxm, right=7mm of ncsaenct] { Base Enclave Test Spec };
    \node (ccenct) [docboxm, right =7mm of baseenct ] { Comm Cluster Test Spec };


    \node (dbbt) [docboxm, right =7mm of ccenct ] { Data BackBone  Test Spec };
    \node (datast) [docboxm, right=7mm of dbbt] { Data Services Test Spec};

    \node (scipt) [docboxm, right=7mm of datast] {Science Platform  Test Spec };
    \node (l1t) [docboxm, right=7mm of scipt] { L1 Test Spec };
    \node (l2t) [docboxm, right=7mm of l1t] { L2 Test Spec };
    \node (infrat) [docboxm, right=7mm of l2t] { DBB Infrastructure  Test Spec };

    \node (pltr) [docboxm, above=5mm of l2t] { L2 Test Reports };
    \node (qservtr) [docboxm, above=5mm of datast] { DBB Test Reports };
    \node (suittr) [docboxm, above=5mm of scipt] { Science Platform Test Reports };
   ...
   \draw[dline] (srd.east) to (lsr.west);
   \draw[dline] (lsr.east) to (oss.west);
...
%long line for test specs
   ...
   \draw[tline] (scipt.south) to (suitr.north);
%reports
   \draw[dline] (l2t.north) to (pltr.south);
   \draw[dline] (datast.north) to (qservtr.south);
   \draw[dline] (scipt.north) to (suittr.south);
