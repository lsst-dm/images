...
%%%
	\node (srp) [above=15mm of scireq.north] {};
	\node (dmreq) [docbox, left=2.5mm of srp] { DM System Requirements  LSE-61 (DMSR)};
	\node (prod) [docbox, right=2.5mm of srp] { LSST Data Products  LSE-163 (DPDD)};
	\node (svtp) [docbox, left=44mm of dmreq.west] { DM  Validation \& Test Plan LDM-503 (SVTP)};
    \node (pmp) [docbox, right=7mm of prod.east] { DM PMP LDM-294};
    \node (crd) [docboxm, text width=38mm,  right=35mm of pmp.east] {Config/Release/Deploy Management };
   ...
   \draw[dline] (prod.north) to (compd.south);
   \draw[dline] (scireq.north) to (dmreq.south);
   \draw[dline] (scireq.north) to (prod.south);
