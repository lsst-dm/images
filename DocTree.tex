...
\usepackage[paperwidth=42cm,paperheight=22cm,
left=-3mm,top=3mm,bottom=0mm,right=0mm,
...
%  LEGEND
	...
	\node (date) [right= 15cm of ttext] {\today};
	...
	\node (ok) [docbox, left=5mm of upd] { In CCB/Released};
%%%
	...
	\node (ats) [docboxu, above=54mm of svtp.north] {DM Science Acceptance Test Specification LDM-639};
    \node (pmp) [docbox, right=120mm of dpdd.east] { DM PMP LDM-294};
    \node (pmpp) [above=20mm of pmp] {};
    \node (crd) [docbox, text width=38mm,  right=25mm of pmp.east] {DM Releases for Verificaiton/Integration LDM-564 };
    ...
    \node (compd)  [docbox, left=7mm of pmpp] { Component Architecture  LDM-148};
%LDM-154, DB LDM-155, Middleware LDM-156%%
%%%%%% REQUIREMENTS %%%%%
    \node (scipr) [docbox, left=120mm of compd] { Science Platform Requirements LDM-554};
    \node (dbbr) [docbox, right=5mm of scipr] { Data BackBone Requirements LDM-635};
    \node (sciapr) [docbox, right=5mm of dbbr] { L1 (prompt)  Requirements LDM-602};
    \node (scidrpr) [docbox, right=5mm of sciapr] { L2 (DRP)  Requirements LDM-562};
   ...
   % \node (suitc) [docboxu, above=5mm of scipc] { SUIT Design LDM-131};
    ...
    \node (userd)  [docbox, right=15mm of compd] { User Documentation };

    %%%%%%%%%%% TEST SPECS %%%%%%%%%%%%%%
    \node (ncsaenct) [docboxm, above=22mm of auxicd ] { NCSA Enclave Test Spec LDM-532};
    \node (baseenct) [docbox, right=7mm of ncsaenct] { DM Raw Image Archiving Service Test Spec LDM-538};
    \node (ccenct) [docboxm, right =7mm of baseenct ] { Comm Cluster Test Spec LDM-541};


    \node (qservt) [docbox, right=7mm of ccenct] { Qserv test spec LDM-552 } ;

    \node (datast) [docboxm, right=7mm of qservt] { Data Services Test Spec LDM-536};
    \node (dbbt) [docboxm, right =7mm of datast ] { Data BackBone  Test Spec LDM-535 };

    \node (scipt) [docbox, right=7mm of dbbt] {Science Platform  Test Spec LDM-540 };
    \node (l1t) [docbox, right=7mm of scipt] { L1 Test Spec LDM-533 };
  ...
  %%%%%%%%%% test reports %%%%%%%%%%%%%%%%%%%
    ...
    \node (qservtr) [docbox, above=25mm of qservt] { Qserv Test Reports };
    \node (datastr) [docboxm, above=25mm of datast] { Data Services Test Reports };
    \node (sciptr) [docbox, above=25mm of scipt] { Science Platform Test Reports };
    \node (l1tr) [docbox, above=25mm of l1t] { L1 Test Reports };
    \node (infratr) [docbox, right=8mm of l2t] { Infrastructure Test Reports };
   ...
   \node (dbbrp) [below=0.85 of dbbr] {};
   \draw[dline] (dbbrp) to  (dbbr.south);
   \node (mwrp) [below=0.85 of mwr] {};
   \draw[dline] (mwrp) to (mwr.south);
   \node (sciaprp) [below=0.85 of sciapr] {};
   \draw[dline] (sciaprp) to (sciapr.south);
   \node (scidrprp) [below=0.85 of scidrpr] {};
   \draw[dline] (scidrprp) to (scidrpr.south);

   %%% DESSIGN LINES %%%%
   \draw[dline] (dbr.north)  ++(-0.3,0.0) --  (dbc.south);
   \draw[dline] (mwr.north)  ++(-0.3,0.0) --  (mwc.south);
   \draw[dline] (scipr.north)  ++(-0.3,0.0) --  (scipc.south);
   \draw[dline] (sciapr.north)  ++(-0.3,0.0) --  (plc.south);
   \draw[dline] (scidrpr.north)  ++(-0.3,0.0) --  (plc.south);
   ...
   \draw[dlines] (compd.north) -- (netccp.north);
   ...
   \node (qservtp) [left=3mm of qservt] {};
   \node (qservtb) [below=10mm of qservtp.east] {};
   \draw[dline] (qservtb.west) |-  (qservt.west);
...
%long line for test specs
   ...
   \draw[dlines] (baseenctpb2.west) -- (l2tpb.west);
   \draw[dlines] (baseenctpb2.west) -- (ncsaenctpb.west);
   \draw[dlines] (svtp.north) -- (ats.south);
   \draw[dlines] (ats.north) |- (baseenctpb2.west);
   ...
   \draw[tlines] (qservtr.north) -- ++(0,0.6);
   \draw[tlines] (infratr.east) -- ++(0.7,0);
   ...
   \draw[tline] (infratr.south) to (netc.north);
