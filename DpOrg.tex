...
\usepackage[paperwidth=29cm,paperheight=13.0cm,
...
\begin{tikzpicture}[node distance=0mm]
    ...
    \node (dpad) [divbox] {\textbf{AD Data Management} William O'Mullane};
    ...
    \node (udfa) [psbox, below=1cm of ddpad, text width=4cm] {\textbf{Advisor \\US Data Facility}\\ Adam Bolton};
...
%
	...
	\node(sumw)[arcbox,below=6cm of dpad]{\textbf{ Data Abstraction } \\Tim Jenness} ;
	\node(alp)[arcbox,right=2mm of sumw]{\textbf{ Data Production } \\Yusra AlSayyad} ;
	\node(ds)[arcbox,right=2mm of alp]{\textbf{ Data Services} \\Frossie Economou} ;
	\node(infra)[arcbox,left=2mm of sumw]{\textbf{Infrastructure/Data Facilities} \\ Richard Dubois} ;
	\node(devops)[arcbox,left=2mm of infra]{\textbf{Data Acquisition/LHN} \\ Cristi\'{a}n Silva} ;
	%\node(wan)[gbox,text width=30mm,below=2cm of devops]{\textbf{Wide Area Network} \\ Cristi\'an Silva} ;
	%\node(sinfra)[gbox,text width=30mm,right=6mm of wan]{\textbf{SLAC Infra} \\ Wei Yang } ;
	%\node(datac)[gbox,text width=30mm,right=2mm of sinfra]{\textbf{Data Curation} \\ Brandon White } ;
	%\node(avs)[gbox,text width=30mm,right=2mm of datac]{\textbf{Alert Vetting Sys} \\ Michael Schneider } ;
	%\node(exec)[gbox,text width=30mm,right=6mm of avs]{\textbf{Proc. Execution} \\ open} ;
	%\node(adb)[gbox,text width=30mm,right=6mm of exec]{\textbf{Advanced DB} \\ Fritz Mueller } ;
	%\node(square)[gbox,right=2mm of adb]{\textbf{ Service Quality and Reliability Engineering } \\Frossie Economou} ;
%Lines ..
...
\draw[line] (ds.north)-- ++(0,0.5)  -| (dpad.south);
\draw[line] (alp.north) -- ++(0,0.5) -| (dpad.south);
\draw[line] (infra.north)-- ++(0,0.5) -| (dpad.south);
\draw[line] (devops.north)-- ++(0,0.5)  -| (dpad.south);

%  \draw[line] (exec.north) --  (alp.south);
%\draw[line] (wan.north)-- ++(0,0.5)  -| (devops.south);
%\draw[line] (adb.north)-- ++(0,0.5)  -| (ds.south);
%\draw[line] (square.north)-- ++(0,0.5)  -| (ds.south);
