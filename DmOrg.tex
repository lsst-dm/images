...
% SYSENG
            \node (sysengg) [mbox, text width=50mm, rectangle split, rectangle split parts=2, left=3.1cm of ccbVpos.north]
                {
                 Core DMCCB
    ...
    \node (man) [mbox, below=1mm of softarc, text width=39mm] {\textbf{ DM Manager }\\ William O'Mullane };
%%%% end SYSENG

    \node (cm) [mbox, left=3.1cm of rmVpos , text width=45mm] {\textbf{Config/Release Manager }\\ Gabriele Comoretto};
    \node (pconp) [below=5mm of dmadv] {};
    \node (pcon) [mbox, right=70mm of pconp , text width=45mm] {\textbf{Project Control/Scheduler }\\ Kevin Long};
    \node (admin) [mbox, below=5mm of pcon, text width=45mm] {\textbf{DM Admin}\\ Aime Brown Woest\\ Erin Calrson};
    ...
    \dmmnode[right=10mm of mwareVpos.north]{mware}{Middleware}{Tim Jenness}{Robert Gruendl}{psbox} ;
    ...
    \node (drp) [mbox, text width=36mm, rectangle split, rectangle split parts=2, base left=2mm of das]
	 ...
	 \dmnode[base right=2mm of square]{infra}{Data Facility }{Michelle Butler}{Robert Gruendl}{psbox} ;
	 \dmnode[base right=2mm of infra]{lhn}{IT, LHN, Base Site}{Cristian Silva}{Michelle Butler}{pobox} ;

 %% GROUPS ..

    \node (dmsg) [psbox, above=1.4cm of pcon, minimum height=12mm] {\textbf{DM Subsystem Science Team}};
...
%dm lines second number is the proportional turning point of the line
  % \draw[line] (alerts.north) -- ++(0,0.3) -| (dmpm.south);
   ...
   \draw[line] (cm.east) -| (rmVpos.north);
